...
public class HiredStudentsService {
    ...
    @Autowired
    public HiredStudentsService(HiredStudentsRepository hiredStudentsRepository, UserRepository userRepository,
    ...
    public void addNewEntry(HiredStudentsDTO hiredStudentsDTO) throws Exception {
        ...
        if (user != null)
            hiredStudentsRepository.save(student);
        else {
    ...
    public List<Job> getRelevantJobs(String rollNo) {
        ...
        for (int job_type : student_job_type) {
            if (job_type == 1)
                student_jobs.add("6 Month Intern");
            else if (job_type == 2)
                student_jobs.add("11 Month Intern");
            else if (job_type == 3)
                student_jobs.add("Fulltime");
            else if (job_type == 4)
                student_jobs.add("6 Month Intern and Fulltime");
            else if (job_type == 5)
                student_jobs.add("11 Month Intern and Fulltime");
            else if (job_type == 6 || job_type == 7)
                student_jobs.add("Summer Intern");
        }
        ...
        for (Job j : allJobs) {
            ...
            for (String job_type : job_types) {
                ...
                if (job_separation.length == 2) {
                    if (student_jobs.contains(job_separation[0]) || student_jobs.contains(job_separation[1]))
                        continue;
                }
    ...
    public List<JobDTOReturn> getRelevantJobsByCollegeEmail(String collegeEmail) {
        ...
        if (userOptional.isPresent()) {
            ...
            for (Job j : fetchedJobs) {
                JobDTOReturn temp = new JobDTOReturn(j.getJobId(), j.getJobRole(), j.getCompanyId(), "",
                        j.getAppDeadline().toString(), j.getStatus().toString(), j.getSalaryBreakup(),
                        j.getEligibility(), j.getAddiInfo(), j.getSpocDetails());
    ...
    public Set<String> getRelevantJobEmails(JobDTO jobDTO) {
        ...
        for (String job_type : eligibility) {
            ...
            if (job_type.equals("6 Month Intern"))
                job_types.add(1);
            else if (job_type.equals("11 Month Intern"))
                job_types.add(2);
            else if (job_type.equals("Fulltime"))
                job_types.add(3);
            else if (job_type.equals("6 Month Intern and Fulltime"))
                job_types.add(4);
            else if (job_type.equals("11 Month Intern and Fulltime"))
                job_types.add(5);
            else if (job_type.equals("Summer Intern"))
                job_types.add(6);
        }
        ...
        outer: for (String rollNo : hiredStudentJobTypes.keySet()) {
            ...
            if (hiredStudentJobTypes.get(rollNo).contains(4) || hiredStudentJobTypes.get(rollNo).contains(5)
                    || (hiredStudentJobTypes.get(rollNo).contains(1) && hiredStudentJobTypes.get(rollNo).contains(3))
                    || (hiredStudentJobTypes.get(rollNo).contains(2) && hiredStudentJobTypes.get(rollNo).contains(3))) {
                continue;
            }
